...
_setup_compinit_cache() {
    local -a _fpath_stamps _fpath_dirs _fpath_mtime _reval_recent
    local _fpath_dir _fpath_fingerprint _fpath_recorded _compinit_rebuild
...
# Recheck the fingerprint after acquiring the lock, including on warm reads.
# If locking is unavailable or times out, initialize in memory without
# reading or writing shared dumps. Tool integrations similarly skip their
# cache on lock failure rather than racing an active writer.
_with_cache_lock "$ZSH_COMPDUMP" _setup_compinit_cache || compinit -i -D
